...
namespace bns
{
    ...
    void WebGPUUnlitSpriteRenderer::SetupIndexBuffer()
    {
    ...
    WebGPUUnlitSpritePipeline &WebGPUUnlitSpriteRenderer::GetPipeline(WebGPUTexture2D *texture)
    {
        ...
        if (m_currentDrawPipelines.count(textureId) == 0)
        {
            ...
            {
                WebGPUUnlitSpritePipeline *pipeline = m_allocatedPipelines[textureId].top();
            ...
            WebGPUUnlitSpritePipeline *pipeline = WebGPUUnlitSpritePipeline::Create(
                m_device, texture,
    ...
    void WebGPUUnlitSpriteRenderer::Initialize()
    {
    ...
    void WebGPUUnlitSpriteRenderer::BeginFrame()
    {
        ...
        {
            std::stack<WebGPUUnlitSpritePipeline *> &pipelineStack = keyValuePair.second;
    ...
    void WebGPUUnlitSpriteRenderer::EndFrame()
    {
        ...
        for (auto keyValuePair : m_currentDrawPipelines)
        {
            ...
            while (!pipelineStack.empty())
            {
                ...
                WebGPUUnlitSpritePipeline *spritePipeline = pipelineStack.top();
                pipelineStack.pop();
