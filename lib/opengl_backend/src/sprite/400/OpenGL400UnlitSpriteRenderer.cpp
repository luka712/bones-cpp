...
namespace bns
{
    ...
    OpenGL400UnlitSpriteRenderer::OpenGL400UnlitSpriteRenderer(Renderer *renderer)
        : SpriteRenderer(), m_renderer(static_cast<OpenGLRenderer *>(renderer))
    {
        BrightnessThreshold = 0.3f;
    }

    void OpenGL400UnlitSpriteRenderer::SetupIndexBuffer()
    {
    ...
    VAOBuffer* OpenGL400UnlitSpriteRenderer::CreateVertexBuffer()
    {
    ...
    OpenGL400UnlitSpritePipeline &OpenGL400UnlitSpriteRenderer::GetPipeline(OpenGLTexture2D *texture)
    {
        ...
        if (m_currentDrawPipelines.count(textureId) == 0)
        {
            ...
            {
                OpenGL400UnlitSpritePipeline *pipeline = m_allocatedPipelines[textureId].top();
            ...
            OpenGL400UnlitSpritePipeline *pipeline = OpenGL400UnlitSpritePipeline::Create(texture);
            m_currentDrawPipelines[textureId].push(pipeline);
    ...
    void OpenGL400UnlitSpriteRenderer::Initialize()
    {
        // setup camera
    ...
    void OpenGL400UnlitSpriteRenderer::BeginFrame()
    {
        ...
        {
            std::stack<OpenGL400UnlitSpritePipeline *> &pipelineStack = keyValuePair.second;
    ...
    void OpenGL400UnlitSpriteRenderer::Draw(Texture2D *texture, const Rect &drawRect, const Rect &sourceRect, const Color &tintColor,
                                         f32 rotationInRadians, const Vec2f &rotationOrigin)
    {
        OpenGLTexture2D *openGLTexture = static_cast<OpenGLTexture2D *>(texture);
        OpenGL400UnlitSpritePipeline &pipeline = GetPipeline(openGLTexture);
    ...
    void OpenGL400UnlitSpriteRenderer::DrawString(SpriteFont *font,
    ...
    void OpenGL400UnlitSpriteRenderer::EndFrame()
    {
        ...
        for (auto keyValuePair : m_currentDrawPipelines)
        {
            ...
            while (!pipelineStack.empty())
            {
                ...
                OpenGL400UnlitSpritePipeline *spritePipeline = pipelineStack.top();
                pipelineStack.pop();
                ...
                glUniformMatrix4fv(spritePipeline->GetProjectionViewLocation(), 1, GL_FALSE, &m_camera.ProjectionViewMatrix.R0C0);
