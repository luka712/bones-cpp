...
namespace bns
{
    ...
    MetalUnlitSpriteRenderer::MetalUnlitSpriteRenderer(Renderer *renderer)
    ...
    void MetalUnlitSpriteRenderer::SetupIndexBuffer()
    {
    ...
    MetalUnlitSpritePipeline &MetalUnlitSpriteRenderer::GetPipeline(MetalTexture2D *texture)
    {
        ...
        if (m_currentDrawPipelines.count(textureId) == 0)
        {
            ...
            {
                MetalUnlitSpritePipeline *pipeline = m_allocatedPipelines[textureId].top();
            ...
            MetalUnlitSpritePipeline *pipeline = MetalUnlitSpritePipeline::Create(*m_renderer, texture);
            m_currentDrawPipelines[textureId].push(pipeline);
    ...
    void MetalUnlitSpriteRenderer::Initialize()
    {
        // setup camera buffer
    ...
    void MetalUnlitSpriteRenderer::BeginFrame()
    {
        ...
        {
            std::stack<MetalUnlitSpritePipeline *> &pipelineStack = keyValuePair.second;
    ...
    void MetalUnlitSpriteRenderer::Draw(Texture2D *texture, const Rect &drawRect, const Rect &sourceRect, const Color &tintColor,
                                   f32 rotationInRadians, const Vec2f &rotationOrigin)
    {
        MetalTexture2D *metalTexture = static_cast<MetalTexture2D *>(texture);
        MetalUnlitSpritePipeline &pipeline = GetPipeline(metalTexture);
    ...
    void MetalUnlitSpriteRenderer::DrawString(SpriteFont *font,
    ...
    void MetalUnlitSpriteRenderer::EndFrame()
    {
        ...
        for (auto keyValuePair : m_currentDrawPipelines)
        {
            ...
            while (!pipelineStack.empty())
            {
                ...
                MetalUnlitSpritePipeline *spritePipeline = pipelineStack.top();
                pipelineStack.pop();
