...
namespace bns
{
	void VulkanSwapChainKHRUtil::LogTransformBits(VkSurfaceTransformFlagsKHR transformBits)
	{
		...
		LOG("Identity: " + std::to_string(transformBits & VK_SURFACE_TRANSFORM_IDENTITY_BIT_KHR));
		LOG("Rotate 90: " + std::to_string(transformBits & VK_SURFACE_TRANSFORM_ROTATE_90_BIT_KHR));
		LOG("Rotate 180: " + std::to_string(transformBits & VK_SURFACE_TRANSFORM_ROTATE_180_BIT_KHR));
		LOG("Rotate 270: " + std::to_string(transformBits & VK_SURFACE_TRANSFORM_ROTATE_270_BIT_KHR));
		LOG("Horizontal mirror: " + std::to_string(transformBits & VK_SURFACE_TRANSFORM_HORIZONTAL_MIRROR_BIT_KHR));
		LOG("Horizontal mirror rotate 90: " + std::to_string(transformBits & VK_SURFACE_TRANSFORM_HORIZONTAL_MIRROR_ROTATE_90_BIT_KHR));
		LOG("Horizontal mirror rotate 180: " + std::to_string(transformBits & VK_SURFACE_TRANSFORM_HORIZONTAL_MIRROR_ROTATE_180_BIT_KHR));
		LOG("Horizontal mirror rotate 270: " + std::to_string(transformBits & VK_SURFACE_TRANSFORM_HORIZONTAL_MIRROR_ROTATE_270_BIT_KHR));
		LOG("Inherit: " + std::to_string(transformBits & VK_SURFACE_TRANSFORM_INHERIT_BIT_KHR));
	}

	void VulkanSwapChainKHRUtil::LogAlphaCompositeBits(VkCompositeAlphaFlagsKHR alphaCompositeBits)
	{
		LOG("Alpha composite bits:");
		LOG("Opaque: " + std::to_string(alphaCompositeBits & VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR));
		LOG("Pre-multiplied: " + std::to_string(alphaCompositeBits & VK_COMPOSITE_ALPHA_PRE_MULTIPLIED_BIT_KHR));
		LOG("Post-multiplied: " + std::to_string(alphaCompositeBits & VK_COMPOSITE_ALPHA_POST_MULTIPLIED_BIT_KHR));
		LOG("Inherit: " + std::to_string(alphaCompositeBits & VK_COMPOSITE_ALPHA_INHERIT_BIT_KHR));
	}

	void VulkanSwapChainKHRUtil::LogSupportedImageUses(VkImageUsageFlags supportedImageUses)
	{
		LOG("Supported image uses:");
		LOG("Transfer source: " + std::to_string(supportedImageUses & VK_IMAGE_USAGE_TRANSFER_SRC_BIT));
		LOG("Transfer destination: " + std::to_string(supportedImageUses & VK_IMAGE_USAGE_TRANSFER_DST_BIT));
		LOG("Sampled: " + std::to_string(supportedImageUses & VK_IMAGE_USAGE_SAMPLED_BIT));
		LOG("Storage: " + std::to_string(supportedImageUses & VK_IMAGE_USAGE_STORAGE_BIT));
		LOG("Color attachment: " + std::to_string(supportedImageUses & VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT));
		LOG("Depth stencil attachment: " + std::to_string(supportedImageUses & VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT));
		LOG("Transient attachment: " + std::to_string(supportedImageUses & VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT));
		LOG("Input attachment: " + std::to_string(supportedImageUses & VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT));
	}

	void VulkanSwapChainKHRUtil::LogSurfaceCapabilities(VkSurfaceCapabilitiesKHR surfaceCapabilities)
	{
		LOG("Surface capabilities:");
		LOG("Min image count: " + surfaceCapabilities.minImageCount);
		LOG("Max image count: " + surfaceCapabilities.maxImageCount);
		LOG("Current extent: " + std::to_string(surfaceCapabilities.currentExtent.width) + " x " + std::to_string(surfaceCapabilities.currentExtent.height));
		LOG("Min image extent: " + std::to_string(surfaceCapabilities.minImageExtent.width) + " x " + std::to_string(surfaceCapabilities.minImageExtent.height));
		LOG("Max image extent: " + std::to_string(surfaceCapabilities.maxImageExtent.width) + " x " + std::to_string(surfaceCapabilities.maxImageExtent.height));
		LOG("Max image array layers: " + surfaceCapabilities.maxImageArrayLayers);
		LOG("Supported transforms: " + surfaceCapabilities.supportedTransforms);
		LOG("Current transform: " + surfaceCapabilities.currentTransform);
	}
