...
namespace bns
{
    ...
    OpenGLESUnlitSpriteRenderer::OpenGLESUnlitSpriteRenderer(Renderer *renderer)
        : SpriteRenderer(), m_renderer(static_cast<OpenGLESRenderer *>(renderer))
    {
        BrightnessThreshold = 0.3f;
    }

    void OpenGLESUnlitSpriteRenderer::SetupIndexBuffer()
    {
        ...
        m_indexBuffer = OpenGLESUtil::Buffer.CreateIndexBuffer(data);
    }

    OpenGLESVAOBuffer* OpenGLESUnlitSpriteRenderer::CreateVertexBuffer()
    {
        ...
        GLuint buffer;
        GLuint vao = OpenGLESUtil::VAO.Create(vertexBufferLayouts, &buffer, GL_DYNAMIC_DRAW);
        OpenGLESVAOBuffer *vaoBuffer = new OpenGLESVAOBuffer();
        vaoBuffer->VAO = vao;
        vaoBuffer->VBO = buffer;
        return vaoBuffer;
    }

    OpenGLESUnlitSpritePipeline &OpenGLESUnlitSpriteRenderer::GetPipeline(OpenGLESTexture2D *texture)
    {
        ...
        if (m_currentDrawPipelines.count(textureId) == 0)
        {
            ...
            {
                OpenGLESUnlitSpritePipeline *pipeline = m_allocatedPipelines[textureId].top();
            ...
            OpenGLESUnlitSpritePipeline *pipeline = OpenGLESUnlitSpritePipeline::Create(texture);
            m_currentDrawPipelines[textureId].push(pipeline);
    ...
    void OpenGLESUnlitSpriteRenderer::BeginFrame()
    {
        ...
        {
            std::stack<OpenGLESUnlitSpritePipeline *> &pipelineStack = keyValuePair.second;
    ...
    void OpenGLESUnlitSpriteRenderer::Draw(Texture2D *texture, const Rect &drawRect, const Rect &sourceRect, const Color &tintColor,
                                         f32 rotationInRadians, const Vec2f &rotationOrigin)
    {
        OpenGLESTexture2D *OpenGLESTexture = static_cast<OpenGLESTexture2D *>(texture);
        OpenGLESUnlitSpritePipeline &pipeline = GetPipeline(OpenGLESTexture);
    ...
    void OpenGLESUnlitSpriteRenderer::DrawString(SpriteFont *font,
                                               const std::string &text,
                                               const Vec2f &position,
                                               const Color &tintColor,
                                               const f32 scale)
    {
        OpenGLESTexture2D *OpenGLESTexture = static_cast<OpenGLESTexture2D *>(font->GetTexture());
    ...
    void OpenGLESUnlitSpriteRenderer::EndFrame()
    {
        std::stack<OpenGLESVAOBuffer*> tempVertexBufferStack;
        ...
        for (auto keyValuePair : m_currentDrawPipelines)
        {
            ...
            while (!pipelineStack.empty())
            {
                ...
                OpenGLESUnlitSpritePipeline *spritePipeline = pipelineStack.top();
                pipelineStack.pop();
                m_allocatedPipelines[textureId].push(spritePipeline);

                GLuint program = spritePipeline->GetProgram();
                const OpenGLESTexture2D *texture = spritePipeline->GetTexture();

                // allocate or get vertex buffer.
                OpenGLESVAOBuffer* vaoBuffer = nullptr;
